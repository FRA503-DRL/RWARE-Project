...
class Warehouse(gym.Env):
    ...
    def _use_slow_obs(self):
        ...
        self._obs_bits_for_self = 5 + len(Direction)
            ...
            OrderedDict(
                {
                    ...
                    "carrying_shelf": gym.spaces.MultiBinary(1),
                    "shelf_requested": gym.spaces.MultiBinary(1),  #  ← เพิ่ม 1
                    "direction": gym.spaces.Discrete(4),
                    "on_highway": gym.spaces.MultiBinary(1),
                }
            )
    ...
    def _get_default_obs(self, agent):
        ...
        if self.fast_obs:
            ...
            # ถือ shelf ที่เป็น requested หรือไม่
            carrying = agent.carrying_shelf
            # is_requested = (
            #     int(carrying in self.request_queue) if carrying is not None else 0
            # )

            is_carrying  = int(carrying is not None)      # 1 = กำลังถือ
            is_requested = int(carrying in self.request_queue) if carrying else 0

            obs.write([agent_x, agent_y, is_carrying, is_requested])


            obs.write([agent_x, agent_y, is_requested])
    ...
    def step(
        ...
        # -----------------------------------
        #  ก่อนลูป for agent in self.agents:
        reward_flags = [
            {
                "walk_wrong": False,
                "pick_requested": False,
                "pick_unrequired": False,
                "return_after_delivery": False,
                "deliver_to_goal": False,
                "drop_wrong": False,
            }
            for _ in range(self.n_agents)
        ]
        ...
        for agent in self.agents:
            ...
            if agent.req_action == Action.FORWARD:
                ...
                # 1) เดินชน shelf ที่ไม่ request → ลบ 0.05
                shelf_id = self.grid[_LAYER_SHELFS, new_y, new_x]
                if shelf_id:
                    shelf = self.shelfs[shelf_id - 1]
                    # if shelf not in self.request_queue:
                        # reward -= 0.05
                        # reward_flags[agent.id - 1]["walk_wrong"] = True

                agent.x, agent.y = new_x, new_y
                if agent.carrying_shelf:
                    agent.carrying_shelf.x, agent.carrying_shelf.y = agent.x, agent.y

                # # เพิ่ม proximity reward
                # if (
                #     agent.carrying_shelf and
                #     agent.carrying_shelf in self.request_queue and
                #     any(abs(agent.x - gx) + abs(agent.y - gy) <= 2 for gy, gx in self.goals)
                # ):
                #     reward += 0.1
                ...
                if (
                    agent.carrying_shelf and
                    agent.carrying_shelf in self.request_queue and
                    self._is_highway(agent.x, agent.y)          # <- เงื่อนไขใหม่
                ):
                    distance = min(
                        abs(agent.x - gx) + abs(agent.y - gy)
                        for gy, gx in self.goals
                    )
                    proximity_reward = max(0, (10 - distance)) * 0.01  # ใกล้กว่า → โบนัสมากสุด 0.4
                    reward += proximity_reward

                # if agent.carrying_shelf and agent.carrying_shelf in self.request_queue:
                #     distance = min(abs(agent.x - gx) + abs(agent.y - gy) for gy, gx in self.goals)
                #     proximity_reward = max(0, (20 - distance)) * 0.02  # ยิ่งใกล้ยิ่งได้มากสุด 0.2
                #     reward += proximity_reward
                ...
                if shelf_id:
                    ...
                    # 2) หยิบ shelf ที่เป็น requested → ได้ 0.5
                    if shelf in self.request_queue:
                        reward += 0.1
                        reward_flags[agent.id - 1]["pick_requested"] = True
                    # ลงโทษถ้าหยิบ shelf ที่ไม่ใช่ requested
                    if shelf not in self.request_queue:
                        reward -= 0.05
                        reward_flags[agent.id - 1]["pick_unrequired"] = True
                ...
                if not self._is_highway(agent.x, agent.y):
                    ...
                    if agent.has_delivered:
                        reward += 0.5
                        agent.has_delivered = False
                        reward_flags[agent.id - 1]["return_after_delivery"] = True
                    elif shelf in self.request_queue:
                        # ถ้าวางผิดที่ (ไม่ใช่ goal) → -0.5
                        if (agent.x, agent.y) not in self.goals:
                            reward -= 0.1
                            reward_flags[agent.id - 1]["drop_wrong"] = True

                # if not self._is_highway(agent.x, agent.y):
                #     # ปล่อย shelf บนพื้น
                #     agent.carrying_shelf = None

                #     # ถ้าเพิ่งส่งสำเร็จ แล้วปล่อย → ได้อีก 0.2
                #     if agent.has_delivered:
                #         reward += 0.2

                #     agent.has_delivered = False

            # เพิ่ม time penalty ทุก agent ทุก step
            # reward -= 0.001
        ...
        for y, x in self.goals:
            ...
            if self.reward_type == RewardType.GLOBAL:
                ...
                rewards[agent_id - 1] += 4
                reward_flags[agent_id - 1]["deliver_to_goal"] = True
        ...
        info = self._get_info()
        info["reward_flags"] = reward_flags         # ← เพิ่มบรรทัดนี้
